...
high_radii = []
for b in pc.bands:
    pc.set_band(pc.get_box_filter(b, outside=True), "fft_pass1")
    gt.quick_render(pc.band("fft_pass1"))
    gt.quick_render(pc.get_box_filter("fft_pass1", outside=True))
    #gt.quick_render(pc.get_box_filter(b, outside=True))
    exit(0)
